(function() {
    ...
    console.log(' AI Text TinyMCE - Version 2.2 - Démarrage global...');
    ...
    // Vérifier les prérequis - VERSION ÉTENDUE
    function checkPrerequisites() {
        ...
        // Accepter toutes les pages avec éditeur
        if (aiTextVars.isGlobalEnabled) {
            debugLog(' Plugin activé globalement');
            return true;
        }
        
        // Compatibilité avec l'ancienne logique WooCommerce
        if (aiTextVars.isWooCommerce && aiTextVars.isProduct) {
            debugLog(' Page produit WooCommerce détectée');
            return true;
        }
        
        debugLog(' Page d\'édition standard détectée');
        return true;
    }
    ...
    // FONCTIONS WOOCOMMERCE - VERSION CONDITIONNELLE
    function addWooCommerceButtons() {
        ...
        if (!aiTextVars.isProduct) {
            debugLog('ℹ️ Pas une page produit, boutons WooCommerce non nécessaires');
            return false;
        }
        
        debugLog('🔧 Ajout des boutons WooCommerce...');
        
        let titleSuccess = false;
        let slugSuccess = false;
        
        // AJOUTER LE BOUTON TITRE
        try {
    ...
    // Initialisation principale avec support étendu
    function init() {
        debugLog(' Initialisation AI Text Plugin...');
        
        // Toujours initialiser, peu importe le type de page
        debugLog('🔧 Initialisation globale du plugin...');
        
        // Initialiser les boutons WooCommerce si applicable
        if (aiTextVars.isProduct) {
            ...
            initStrategies.forEach((strategy, index) => {
                debugLog(` Exécution stratégie WooCommerce ${index + 1}`);
                strategy();
            });
        }
        
        // TOUJOURS initialiser TinyMCE pour tous les éditeurs
        waitForTinyMCE();
        
        // Support pour Gutenberg sur TOUTES les pages
        if (typeof wp !== 'undefined' && wp.domReady) {
            wp.domReady(function() {
                debugLog('📚 Gutenberg prêt');
                // Attendre que les éditeurs soient chargés
                setTimeout(function() {
                    if (typeof tinymce !== 'undefined' && tinymce.editors) {
                        tinymce.editors.forEach(function(editor) {
                            if (editor && editor.initialized) {
        ...
        // Support spécifique WPBakery amélioré
        if (aiTextVars.hasWPBakery || typeof window.vc !== 'undefined') {
            debugLog('🎨 WPBakery détecté, ajout du support...');
            
            // Attendre que WPBakery soit prêt
            var wpbCheckInterval = setInterval(function() {
                if (window.vc && window.vc.events) {
                    clearInterval(wpbCheckInterval);
                    
                    // Écouter les événements WPBakery
                    window.vc.events.on('shortcodes:add shortcodes:update shortcodes:clone', function() {
                        debugLog('📝 Événement WPBakery détecté');
                        setTimeout(function() {
                            if (typeof tinymce !== 'undefined' && tinymce.editors) {
                                tinymce.editors.forEach(function(editor) {
                                    if (editor && editor.initialized && !editorsProcessed.has(editor.id)) {
                    ...
                    // Aussi écouter l'événement app:ready
                    window.vc.events.on('app:ready', function() {
                        debugLog('🎨 WPBakery app ready');
                        setTimeout(function() {
                            if (typeof tinymce !== 'undefined' && tinymce.editors) {
                                tinymce.editors.forEach(function(editor) {
                                    if (editor && editor.initialized && !editorsProcessed.has(editor.id)) {
        ...
        // Observer les changements DOM pour détecter de nouveaux éditeurs
        const observer = new MutationObserver(function(mutations) {
            mutations.forEach(function(mutation) {
                // Vérifier si de nouveaux éditeurs ont été ajoutés
    ...
    window.aiTextDebug = {
        ...
        // Forcer l'ajout de boutons
        forceAddButtons: () => {
            debugLog(' Forçage de l\'ajout des boutons...');
            if (typeof tinymce !== 'undefined' && tinymce.editors) {
                tinymce.editors.forEach(function(editor) {
                    if (editor && editor.initialized) {
                        editorsProcessed.delete(editor.id); // Reset pour forcer
                        addAIButtonToEditor(editor);
                    }
                });
            }
    ...
    debugLog(' AI Text Plugin chargé avec succès - Version 2.2 - Support global activé');
